# AI PostgreSQL Query Assistant — Design Specification

## Overview

This application is an AI-powered PostgreSQL query assistant designed around a conversational workflow rather than a traditional SQL IDE. The experience should feel closer to ChatGPT than to pgAdmin or a database dashboard.

The interface is centered around a vertically stacked chat layout:
- user message
- rewritten query preview
- assistant response
- retrieval insights

The application’s purpose is to translate natural language into production-quality PostgreSQL (`psql`) queries using a retrieval-augmented generation pipeline.

The user speaks naturally ("show all customers from last month"), while the system:
1. rewrites the query into optimized retrieval language,
2. retrieves relevant PostgreSQL documentation chunks,
3. generates SQL,
4. explains the reasoning behind the generated query.
...
The UI should avoid the heavy "AI tool" aesthetic:
- no neon gradients,
- no glassmorphism overload,
- no noisy dashboards,
- no excessive borders.

Instead, the interface should inherit the same philosophy as Apple's product pages:
- typography-first,
- whitespace-heavy,
- restrained interactions,
- extremely low visual density,
- subtle hierarchy.

The application is not a database admin panel.

It is a thinking interface for SQL generation.
...
The main interface is a conversational timeline similar to ChatGPT:

```text
User Prompt
↓
Query Rewrite Preview
↓
Retrieved Chunks
↓
Generated SQL
...
Each interaction appears as a vertically separated message block.
...
Immediately after the user sends a message, the rewritten retrieval query appears underneath in a muted gray tone.

Example:

```text
SQL SELECT customers with recent orders last 30 days
```

This is NOT a primary message.
...
The rewrite preview communicates:
- what the retrieval engine understood,
- how the query was normalized internally.

This becomes one of the core trust-building features of the application.
...
The SQL block is the visual centerpiece.
...
Visual style:
- left aligned
- no colored AI bubble
- white/pure-black surface depending on theme
- code block uses near-black background
...
One of the application's defining features is a retrieval-inspection surface.

The user should be able to:
- inspect retrieved chunks,
- understand retrieval relevance,
...
- side panel,
- floating dialog,
- collapsible accordion beneath assistant messages.
...
The retrieval panel should feel:
- technical,
- diagnostic,
- transparent.

This is an engineering tool, not a consumer chat app.
...
The query rewrite layer is one of the core architectural components of the system.

Its purpose is to:
- normalize natural language,
- align user intent with PostgreSQL terminology,
...
```text
show all entries in customers table
```

### Internal Rewrite

```text
SQL SELECT all rows from table
SELECT * FROM customers
retrieve all rows using SELECT
```

This rewritten query is displayed underneath the user message in a muted gray tone.
...
Convert casual phrasing into SQL/documentation terminology.

Examples:

| User Input | Rewrite |
|---|---|
| show entries | SELECT rows |
| combine tables | JOIN |
| add data | INSERT |
| remove rows | DELETE |
...
```text
1. SQL SELECT all rows
2. SELECT * FROM table
3. Querying a Table
4. retrieve all rows SQL
```

This improves retrieval recall and ranking quality.

---

# Design Philosophy

## Minimal Engineering Luxury
...
The bottom composer should:
- stay sticky,
- feel weightless,
- resemble ChatGPT's input surface.
...
The retrieval system is a first-class citizen.

Unlike normal AI chats where retrieval is hidden, this app intentionally exposes:
- rewritten queries,
- retrieved chunks,
- retrieval scores,
- source hierarchy.
...
> “This SQL was generated from grounded PostgreSQL knowledge.”

That transparency becomes the product's differentiator.
...
This IS:
- an AI retrieval-powered PostgreSQL reasoning interface,
- optimized for SQL generation and understanding.
...
- appear in a dedicated syntax-highlighted block,
- support copy button,
- support execution in future versions,
- optionally show explanation line-by-line.
...
The application should feel calm, intelligent, and trustworthy.
...
The interface should disappear so the reasoning becomes the focus.
